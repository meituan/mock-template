...
var Mock4Tpl = require('..');
var Handlebars = require('handlebars');
var rEmail = /[\w.]+@\w+\.\w+/;
var rDate = /\d{4}-\d{2}-\d{2}/;
var rTime = /\d{2}:\d{2}:\d{2}/;
var rDatetime = /\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}/;
var rURL = /https?:\/\/\w+\.\w+/;
var rFloat = /(\d+)\.?(\d+)?/;
var rBoolean = /^true|false$/;
var rUpper = /^[A-Z]+$/;
var rLower = /^[a-z]+$/;
var rCapitalize = /^[A-Z][a-z]+$/;
var rTitle = /^([A-Z][a-z]+)( [A-z][a-z]+)+/;
...
<div class="comments">
  ...
  {{/each}}
    ...
    */
    ...
    test.ok(_.isArray(data.comments))
...
{{/list}}
    */
    ...
    test.equal(data.nav[0].url, 'url')
    test.equal(data.nav[0].title, 'title')
...
{{/list}}
    ...
    test.equal(data.array[0].title, 'title')
    test.done()
}
...
{{/each}}
    ...
    test.ok(_.isArray(data.paragraphs))
...
{{/each}}
    ...
    test.ok(_.isArray(data.articles[10]['#comments']))
...
{{/each}}
    ...
    test.ok(_.isArray(data.articles))
    test.ok(_.isArray(data.articles[10].comments))
...
<div bx-tmpl="list" bx-datakey="list">
  <table class="table" bx-name="tables" bx-path="brix/gallery/tables/" id="tables">
    ...
    //     delete(tmp = _.extend({}, item)).children
    //     return tmp
    // }))
    ...
    test.ok(_.isArray(data.list))
    for (var i = 0; i < data.list.length; i++) {
        test.ok(_.isArray(data.list[i].children))
    }
    try {
        data = Mock4Tpl.mock(tpl, options)
        test.ok(_.isArray(data.list))
        data.list.forEach(function(item) {
            test.ok(_.isArray(item.children))
        })
    } catch (error) {
